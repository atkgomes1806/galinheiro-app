...
### `test-embrapa-api.js`
**Objetivo**: Teste básico de integração com a API ClimAPI da Embrapa
- Testa autenticação OAuth 2.0
- Verifica endpoints de variáveis disponíveis  
- Busca dados de temperatura para uma localização
...
### 2. Testar API Embrapa (OAuth)
```bash
cd C:\Projetos\galinheiro-app\galinheiro-app
node scripts/test-embrapa-api.js
```

### 3. Testar API Real (Token Bearer)  
```bash
cd C:\Projetos\galinheiro-app\galinheiro-app
