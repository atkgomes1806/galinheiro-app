...
/**
 * Busca dados da API Embrapa com estratégia de fallback:
 * 1. Tenta Bearer Token primeiro (mais rápido)
 * 2. Se falhar, usa OAuth 2.0 (mais robusto)
 */
async function fetchWithFallback(url) {
  // Token Bearer para tentativa direta
  const bearerToken = '724ecc90-70b1-36c1-b573-c5b01d6173ea';
  
  console.log(`🔄 Tentando Bearer Token primeiro: ${url}`);
  
  try {
    // 1ª Tentativa: Bearer Token
    ...
    console.log('⚠️ Erro Bearer:', error.message);
  }
  
  // 2ª Tentativa: OAuth 2.0
  console.log('🔐 Obtendo token OAuth...');
  const oauthToken = await authService.getValidToken();
  ...
  if (!oauthResponse.ok) {
    throw new Error(`Ambos os métodos falharam. OAuth Status: ${oauthResponse.status}`);
  }
  
  console.log('✅ OAuth funcionou como fallback!');
  return await oauthResponse.json();
}

/**
 * GET /api/weather/data-real
 * Obtém dados climáticos REAIS com fallback Bearer → OAuth
 * 
 * Query params:
 * - lat: latitude (obrigatório)
 * - lon: longitude (obrigatório)
...
router.get('/data-real', async (req, res) => {
  try {
    ...
    console.log('\n🎯 === DADOS REAIS COM FALLBACK BEARER → OAUTH ===');
    console.log(`📍 Buscando clima para: lat=${latitude}, lon=${longitude}`);
    ...
    console.log('🌡️ URL temperatura:', tempURL);
    console.log('💧 URL umidade:', humidityURL);
    
    // Buscar dados com fallback automático
    console.log('\n📊 Buscando temperatura...');
    const tempData = await fetchWithFallback(tempURL);
    
    console.log('\n� Buscando umidade...');
    const humidityData = await fetchWithFallback(humidityURL);
